...
class Editor extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            editor: null,
            cancel_dialog_type: ''  //图片素材未上传提示类型  1:预览；2:发表
        };
    ...
    checkPostArticle() {
        //判断当前素材库中是否还有未添加到文章的图片
        if (tools.getLength(this.MaterialUpload.state.fileQueued) > 0) {
            this.DialogImage.show()
            this.setState({
                cancel_dialog_type: 2
            })
        } else {
            this.Agreement.show()
        }
        ...
        return (
            <form action={"/article/Preview"} method="post" target="_blank" id="preview__form">
                        ...
                        <div className="content__header fixed">
                            <div className="content__header-inner main clear">
                                <div className="content__header-nav">
                                    <a href="/user/article.html">我的文章</a> > {blogid > 0 ?
                                    <a href="/user/article.html?draft=1">草稿箱</a> :
                                ...
                                <div className="content__header-right">
                                    ...
                                    <div className="content__header-btn pullout-fullscreen"
                                         onClick={this.pullOutFullScreen.bind(this)}>
                                        退出全屏
                                    </div>
                                    <div className="content__header-btn post-article"
                                         onClick={this.checkPostArticle.bind(this)}>
                                        发表
                                    </div>
                                </div>
                            </div>
                ...
                <DialogImage
                    ...
                    parent={this}
                    show_type={this.state.cancel_dialog_type}/>
            </form>
        );
    }
        ...
        if (!_this.state.editor) {
			...
			<div id="${_this.state.editor_id}-tools-tips" class="editor__tools-tips-wrap">
				...
				<img class="editor__tools-tips-roow editor__tools-tips-roow2" src=${editor_roow2} />
                    ...
                    jQuery('body').on('click', '.preview__btn-submit', function (e) {
                        e.preventDefault()
                        var r = _this.validationAll(true);
                        if (!r.result) return false;
                        if(tools.getLength(_this.MaterialUpload.state.fileQueued) > 0){
                            _this.DialogImage.show();
                            _this.setState({
                                cancel_dialog_type: 1
                            })
                        }else{
                            _this.sendPreview();
                        }
                    ...
                    doc.on('mouseenter', 'img:not(.edui-faked-video)', function () {
                        ...
                        var src_arr = _img.attr('src').match(/https?:\/\/s[1|2]\.jiguo\.com\/([\w\-]+)\/?/i);
                        if (!src_arr) {
                            return;
                        }
                        ...
                        if (_this.MaterialUpload && _this.MaterialUpload.reinstateImage) {
                            var src_arr = img.attr('src').match(/https?:\/\/s[1|2]\.jiguo\.com\/([\w\-]+)\/?/i);
                            if (!src_arr) {
                                return;
                            }
