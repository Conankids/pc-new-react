...
class CreatePreviewVideo extends React.Component {
        ...
        return (
            <div className="video__wrap">
                ...
                <div className="video__body">
                    {body ? body :
                        <div style={{position: 'relative', height: 350, width: 640, overflow: 'hidden'}}>
                            <div className="previewvideo__msg">
                                {this.state.tips ? this.state.tips :
                ...
                <div className="video__footer">
                    <div className="video__btn video__cansel" onClick={this.props.parent.close.bind(this.props.parent)}>
                        取消
                    </div>
    ...
    convertUrlToFlash(url) {
        url = String(url).replace(/^\s+|\s+$/g, '')
        if (!/^https?:\/\//i.test(url)) {
            return false
        }

        var parseUrlObj = this.parseUrl(url)
        var find = false
        var parseed_url = ''
        var parseAddrUrl = {
                parse_youku_com(url) {
    ...
    convert_url(url, ifram) {
        ...
        if (ifram) {
            ...
            if (url.match("v.qq.com") || url.match("video.qq.com") || url.match("imgcache.qq.com")) {
                url = '//v.qq.com/iframe/player.html?vid=' + getKeyVal(url, 'vid') + '&width=640&height=350&auto=0';
            } else if (url.match("youku.com")) {
                url.match(/https?:\/\/player.youku.com\/player.php\/sid\/(.+)\/v.swf/);
                url = '//player.youku.com/embed/' + RegExp.$1;
            } else if (url.match("bilibili.com")) {
                url.match(/https?:\/\/(?:www\.)?bilibili.com\/video\/av([\da-zA-Z]+)/);
                url = '//player.bilibili.com/player.html?aid=' + RegExp.$1;
