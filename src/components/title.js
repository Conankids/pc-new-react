...
class Title extends Component {
		...
		if (_val && !this.state.title) {
			...
			if (scroll!='not' && this.content__title) {
				jQuery('html,body').animate({
					scrollTop: jQuery(this.content__title).offset().top - 70
				});
			}
			return false;
		}
		...
		return (
		  ...
		  <textarea
				...
				onChange={e => {
					...
					jQuery(this.testLen).html(text);
					var textLen = jQuery(this.testLen).width();
					this.setState({
						title: text,
	...
	componentDidMount() {
		jQuery(this.testLen).html(this.state.title);
		var textLen = jQuery(this.testLen).width();

		var state = {
			length: textLen,
			textareaLen: jQuery(this.textarea).width(),
		};
